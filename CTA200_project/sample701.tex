...
\begin{figure}[ht!]
    \centering
    \includegraphics[width=0.9\linewidth]{varn_residual.png}
...
The argument `max\_step' in the solve\_ivp function was crucial in reducing error. A smaller maximum step led to lower deviation from the true value of $\xi_1$. Figure \ref{fig:good_bad_res} shows the comparison between a solution where the maximum step was not defined and one that had a definite maximum step. Note that the error for $n=1$ is different than that of Figure \ref{fig:varn_res}. We do not know why the program generated different values for $\xi_1$ for different runs.
